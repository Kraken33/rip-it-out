...
- **THEN** the system generates the first Russian story passage grounded in the session title/topic and the learner level, and presents it with a translation input.
...
- **THEN** the system generates a fresh Russian story passage on a topic not already used in this session and presents it as a new round.
...
The system SHALL aggregate candidate constructions from all completed rounds into a single deduplicated import list, capped at `settings.maxImprovements`, when the user finishes the translation activity.
...
#### Scenario: Aggregation respects the improvements cap
- **WHEN** the aggregated candidates exceed `settings.maxImprovements`
- **THEN** the system keeps at most that many items, preferring earliest rounds first.

### Requirement: Translation session artefact

The system SHALL persist a finished translation-story session as a session record carrying the activity marker, all rounds (passage, learner translation, improved version, candidate constructions), timing, and learner-only word metrics.
